...
def about(subject):
    ...
    def check(p):
        words = split(lower(p))
        return any(remove_punctuation(w) in subject for w in words)
    return check
...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words), len(source_words))):
        if typed_words[i] == source_words[i]:
            grade+=1
    return grade*100/len(typed_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in word_list:
        if i == typed_word:
            ...
            tmp = diff_function(typed_word, i, limit)
            if tmp < mini and tmp <= limit:
                mini = tmp
                ret = i
    return ret
...
def furry_fixes(typed, source, limit):
    ...
    def ret(i, sub):
        if sub > limit:
            return max(len(typed), len(source)) - i
        if len(typed) == 0 or len(source) == 0 or i >= len(typed) or i >= len(source):
            return abs(len(typed) - len(source))
        if typed[i] != source[i]:
            sub+=1
            return 1 + ret(i+1, sub)
        else:
            return ret(i+1, sub)
    return ret(0, sub)
    # END PROBLEM 6
    ...
    for n in word_indices:
        ...
        for i in player_indices:
            if times[i][n] < min_t:
                min_t = times[i][n]
                fastest_player = i
        choose[n] = fastest_player
    ans = [[] for _ in player_indices]
    for n in word_indices:
        player = choose[n]
        ans[player].append(words[n])

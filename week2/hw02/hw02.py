...
def interleaved_sum(n, f_odd, f_even):
    ...
    if n == 0:
        ...
        return f_even(n) + interleaved_sum(n-1, f_odd=f_odd, f_even=f_even)
    else:
        return f_odd(n) + interleaved_sum(n-1, f_odd=f_odd, f_even=f_even)
...
def count_dollars(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_using(total, max_denom):
        if total == 0:
            return 1
        if total < 0 or max_denom is None:
            return 0
        return count_using(total, next_smaller_dollar(max_denom)) + count_using(total - max_denom, max_denom)
...
def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    mid = len(s)//2
    return [s[i//2 + (i % 2) * mid] for i in range(len(s))]
